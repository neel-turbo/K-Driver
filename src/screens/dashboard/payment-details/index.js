...
import { PayemntCard } from '../../../components/card/paymentCard';
...
export const PaymentDetails = () => {
  const Item = ({title}) => <PayemntCard label="Paid" />;

  const renderItem = ({item}) => <Item title={item.title} />;

  return (
    <View style={styles.container}>
      <Back title="Payment details" />
      <SearchBar />
      <View>
        <View style={styles.row}>
          <FontIocn name={'checkbox-passive'} size={18} color="#C9CCCF" />
          <Text style={styles.text}>All Successful Ride</Text>
        </View>
